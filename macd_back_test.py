...
def select_Time_AMA(stock_data, stockName,*Avg):
    ...
    # 成本
    print stockName,u'盈利', record_sale + now_count * close_price[-1] - record_buy
    ...
    def fitness_function(chromosome):
        ...
        if fitness == 'abs':
            if strategyName == 'MACD':
                ...
                score = select_Time_MA(stock_data, stockName, *chromosome)
            elif strategyName == 'DMA':
                score = select_Time_DMA(stock_data, stockName, *chromosome)
            elif strategyName == 'TRIX':
                score = select_Time_TRIX(stock_data, stockName, *chromosome)
            elif strategyName == 'AMA':
...
def macdmain(stockList):
    ...
    for stockName in stockList:
        ...
        select_method = [ 'MACD']
        AvgParm=[]
        f = open(stockTxtNewPath, 'r')
        lines = f.readline()
        for i in select_method:
            with open(stockTxtNewPath, 'r') as file_to_read:
                while True:
                    ...
                    if not lines:
                        ...
                        for line in lines:
                            line.split('['*']')
                            if (line[2]==i):
                                AvgParm.append(cal_opt(3, 'abs', i, func, stock_data, stockName))
        # print AvgParm
        f = open(stockTxtNewPath, 'a')
        f.write(str(select_method))
        f.write(str(stockName))
        f.write(str(AvgParm))
        f.write('\n'+'-------------------'+'\n')
        f.close()
        Avg=AvgParm[0]
        ...
        print u'-- MACD 策略 --'
        buysell.append(select_Time_MACD(stock_data, stockName,*Avg))
